...
func getEnvInt(key string, defaultValue int) int {
	if val := os.Getenv(key); val != "" {
		if parsed, err := strconv.Atoi(val); err == nil {
			log.Printf("Config override: %s=%d (default: %d)", key, parsed, defaultValue)
			return parsed
		}
	}
	return defaultValue
...
// Cache Configuration - declare variables but don't initialize yet
var (
	// Memory Management
	MaxTenants           int
	MaxMemoryMB          int
...
// TTL Configuration - declare variables but don't initialize yet
var (
...
// Cleanup Intervals - declare variables but don't initialize yet
var (
...
func init() {
	...
	// Now initialize all config values AFTER .env is loaded

...
// ParseWebhook extracts the Shopify Global ID (GID) from a raw webhook payload.
func (s *ShopifyService) ParseWebhook(body []byte) (string, error) {
	...
	if idStr == "" {
		return "", fmt.Errorf("missing 'id' in webhook payload")
	}
...
func (s *ShopifyService) ReconcileAll(tenantCtx *tenant.Context) (int, int, int, error) {
	...
	for i := 0; i < len(allGIDs); i += batchSize {
		...
		for idx, nodeRaw := range result.Data.Nodes {
			...
			resource := s.transformGraphQLNode(node, category)
			op, err := s.resourceService.UpsertShopifyResource(tenantCtx, resource)
			if err != nil {
				s.logger.System().Error("Failed to reconcile product", "error", err, "gid", requestedGID)
				continue
			}
...
// transformGraphQLNode maps a raw Shopify GraphQL Product node into a high-fidelity ResourceNode.
func (s *ShopifyService) transformGraphQLNode(node map[string]any, category string) *content.ResourceNode {
	// 1. Flatten Images
	if imgs, ok := node["images"].(map[string]any); ok {
		if iEdges, ok := imgs["edges"].([]any); ok {
			flatImages := make([]map[string]any, 0)
...
// SyncProductByGID performs a targeted Storefront GraphQL fetch and synchronization for a single resource.
func (s *ShopifyService) SyncProductByGID(tenantCtx *tenant.Context, gid string) (string, error) {
	token := tenantCtx.Config.ShopifyStorefrontToken
	domain := tenantCtx.Config.ShopifyStoreDomain
	apiVersion := tenantCtx.Config.ShopifyAPIVersion

	if token == "" || domain == "" || apiVersion == "" {
		return "", fmt.Errorf("shopify credentials or api version missing for tenant %s", tenantCtx.TenantID)
	}
	...
	req, err := http.NewRequest("POST", url, bytes.NewBuffer(jsonBody))
	if err != nil {
		return "", fmt.Errorf("failed to create targeted sync request: %w", err)
	}
	req.Header.Set("Content-Type", "application/json")
	req.Header.Set("Shopify-Storefront-Private-Token", token)
	...
	resp, err := client.Do(req)
	if err != nil {
		return "", fmt.Errorf("targeted shopify sync request failed: %w", err)
	}
	defer func() {
		if closeErr := resp.Body.Close(); closeErr != nil {
			s.logger.System().Warn("Failed to close Shopify response body", "error", closeErr)
	...
	if err := json.NewDecoder(resp.Body).Decode(&result); err != nil {
		return "", fmt.Errorf("failed to decode targeted sync response: %w", err)
	}

	if len(result.Errors) > 0 {
		return "", fmt.Errorf("graphql errors in targeted sync: %v", result.Errors)
	}

	if len(result.Data.Nodes) == 0 || result.Data.Nodes[0] == nil {
		return s.resourceService.SyncShopifyDeletion(tenantCtx, gid)
	}

	node, ok := result.Data.Nodes[0].(map[string]any)
	if !ok {
		return "", fmt.Errorf("invalid node data structure for gid %s", gid)
	}

	// Determine category by existing resource or default
	category := "product"
	products, _ := s.resourceService.GetByCategory(tenantCtx, "product")
	for _, p := range products {
		if g, ok := p.OptionsPayload["gid"].(string); ok && g == gid {
	...
	resource := s.transformGraphQLNode(node, category)
	return s.resourceService.UpsertShopifyResource(tenantCtx, resource)
}
